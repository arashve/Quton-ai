...
export function AgentStudioHubModal({
  ...
  const [agents, setAgents] = useState<StudioAgent[]>(() =>
    typeof window !== 'undefined' ? getStudioAgents() : []
  );
  const [currentWs, setCurrentWs] = useState<string>(() =>
    typeof window !== 'undefined' ? getActiveWorkspace() : 'default'
  );
  ...
  useEffect(() => {
    ...
    return subscribeStoreUpdates(handleUpdate);
  }, []);
  ...
  return (
    ...
    >
      ...
      {/* Modal Container: Apple Squircle & Monochrome Surface */}
      <div
        role="dialog"
        aria-modal="true"
        className="relative z-10 w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-[32px] bg-zinc-950 border border-zinc-800 shadow-2xl p-5 sm:p-7 text-zinc-100 flex flex-col font-sans"
      >
        {/* Header */}
        <div className="flex items-center justify-between gap-3 pb-4 mb-4 border-b border-zinc-800">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-2xl bg-zinc-800 border border-zinc-700 flex items-center justify-center text-white text-xl shadow-md">
          ...
          <button
            type="button"
            onClick={onClose}
            className="w-10 h-10 rounded-full bg-zinc-900 hover:bg-zinc-800 text-zinc-400 hover:text-white flex items-center justify-center transition cursor-pointer border border-zinc-800"
            aria-label="بستن"
          >
            <X className="w-5 h-5" />
          </button>
        ...
        <div
          ...
          className={`p-4 rounded-[24px] mb-4 border transition-all cursor-pointer flex items-center justify-between gap-3 ${
            currentWs === 'default'
              ...
              : 'bg-zinc-900/40 hover:bg-zinc-900 border-zinc-800'
          }`}
        >
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {currentWs === 'default' && (
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-mono bg-zinc-800 text-white border border-zinc-700">
                    فضای فعال فعلی
                  </span>
                )}
              </div>
          ...
          <button
            type="button"
            className="px-3.5 py-2 rounded-xl text-xs font-bold bg-white text-zinc-950 hover:bg-zinc-200 transition flex items-center gap-1 shrink-0 cursor-pointer"
          >
            <span>ورود</span>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between gap-3 mb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-2xl bg-zinc-800 border border-zinc-700 text-white">
                      {agent.icon}
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                    ...
                    >
                      <span
                        className={`pointer-events-none inline-block h-6 w-6 transform rounded-full ring-0 transition duration-200 ease-in-out ${
                          agent.isEnabled ? '-translate-x-6 bg-zinc-950' : 'translate-x-0 bg-zinc-400'
                        }`}
                      />
                    </button>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    {agent.isEnabled ? (
                      ...
                      <button
                        type="button"
                        onClick={(e) => handleToggle(agent.id, e)}
                        className="px-3.5 py-2 rounded-xl text-xs font-medium bg-zinc-800 hover:bg-zinc-700 text-zinc-300 transition flex items-center gap-1 cursor-pointer"
                      >
        ...
        <div className="mt-5 pt-4 border-t border-zinc-800 flex items-center justify-between text-xs">
          ...
          <Link
            href="/marketplace"
            onClick={onClose}
            className="text-white hover:text-zinc-300 font-bold flex items-center gap-1 transition underline"
          >
